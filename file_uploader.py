# 创建虚拟环境
# conda create -n pack_file_uploader python=3.7
# 激活虚拟环境
# activate pack_file_uploader 
# Pyinstaller打包
# Pyinstaller -F -i icon.png file_uploader.py

from flask import Flask, request
import os
import qrcode
import base64
from io import BytesIO
import socket
import webbrowser
import time

print('''
//
//                       _oo0oo_
//                      o8888888o
//                      88" . "88
//                      (| -_- |)
//                      0\  =  /0
//                    ___/`---'\___
//                  .' \\|     |// '.
//                 / \\|||  :  |||// \\
//                / _||||| -:- |||||- \\
//               |   | \\\\\  -  /// |   |
//               | \\_|  ''\\---/''  |_/ |
//               \\  .-\\__  '-'  ___/-. /
//             ___'. .'  /--.--\\  `. .'___
//          ."" '<  `.___\\_<|>_/___.' >' "".
//         | | :  `- \\`.;`\\ _ /`;.`/ - ` : | |
//         \\  \\ `_.   \\_ __\\ /__ _/   .-` /  /
//     =====`-.____`.___ \\_____/___.-`___.-'=====
//                       `=---='
//
//
//     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
//               佛祖保佑         永无BUG
//
//     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//                使用完毕之后关闭此窗口
''')
app = Flask(__name__)
app.config['UPLOAD_FOLDER'] = '.'

@app.route('/', methods=['GET', 'POST'])
def upload_file():
    if request.method == 'POST':
        T1 = time.time()
        files = request.files.getlist("files")
        if len(files) == 0:
            return '<script> alert("请先选择文件！！！");window.location.href="/";</script>'
        for file in files:
            filename = file.filename
            file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
        T2 = time.time()
        print('本次上传耗时时间:%s毫秒' % ((T2 - T1)*1000))
        return '<script> alert("上传成功！文件保存在本程序目录下");window.location.href="/";</script>'
    return '''
<!DOCTYPE html>
<html>
<head>
    <title>同WIFI下局域网上传文件</title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <style>
        body {
            display: flex;
            justify-content: center;
            align-items: center;
            flex-direction: column;
            height: 100vh;
            margin: 0;
            background-color: #f0f0f0;
            font-family: Arial, sans-serif;
            padding: 10px;
        }
        h1 {
            color: #333;
            text-align: center;
        }
        h3 {
            color: red;
            text-align: center;
        }
        form {
            display: flex;
            flex-direction: column;
            align-items: center;
            width: 100%;
            max-width: 400px;
        }
        input[type="submit"] {
            margin-top: 10px;
            padding: 5px 20px;
            background-color: #008CBA; 
            border-color: #008CBA;
            color: white;
            text-align: center;
            text-decoration: none;
            font-size: 20px;
            border-radius:15px;
            z-index: 5555;
        }
        input[type="submit"]:hover {
            background-color: #007B9A;
        }
        input[type="file"]::file-selector-button {
            /* font-weight: 500;
            color: white;
            background-color: #008CBA;
            border-color: #008CBA;
            border-radius:15px; */        
            display: none;
        }
        label{
            margin-top: 10px;
            padding: 5px 10px;
            font-weight: 500;
            color: white;
            background-color: #008CBA;
            border-radius:15px;
        }
        @media (max-width: 600px) {
            h1 {
                font-size: 24px;
            }
            h3 {
                color: red;
                font-size: 16px;
            }
        }
        .overlay {
            display: none;
            position: fixed;
            width: 100%;
            height: 100%;
            top: 0;
            left: 0;
            right: 0;
            bottom: 0;
            background-color: rgba(0,0,0,0.5);
            z-index: 6666;
            cursor: pointer;
        }
        #loader {
            display: none;
            border: 16px solid #f3f3f3;
            border-radius: 50%;
            border-top: 16px solid #3498db;
            width: 120px;
            height: 120px;
            -webkit-animation: spin 2s linear infinite;
            animation: spin 2s linear infinite;
            position: fixed;
            z-index: 999999999;    /* 将动画放在所有其他元素之上 */
        }
        @-webkit-keyframes spin {
            0% { -webkit-transform: rotate(0deg); }
            100% { -webkit-transform: rotate(360deg); }
        }
        @keyframes spin {
            0% { transform: rotate(0deg); }
            100% { transform: rotate(360deg); }
        }
    </style>
</head>
<body>
    <h1>同WIFI局域网下上传文件</h1>
    <h2>手机端扫描二维码访问</h2>
    <h3>需要关闭防火墙且所有设备在同一网络，部分手机只支持单文件上传</h3>
    <form method="POST" action="" enctype="multipart/form-data">
'''+img_tag+'''
    <label for="file_select">选择单或多文件</label>
    <input type="file" hidden="hidden" id="file_select" name="files" multiple>
    <input type="submit" value="点击上传" onclick="loading();">
    </form>
    <div id="overlay" class="overlay"></div>
    <div id="loader" class="loader"></div>
    <div style="position: absolute;bottom: 0;width: 100%;height: 1.5rem;text-align: center;z-index: 250;">by h9h</div>
</body>
<script type="text/javascript">
    function loading(){
        document.getElementById("overlay").style.display = "block";
        document.getElementById("loader").style.display = "block";
    }
</script>
</html>'''

if __name__ == '__main__':
    ip_address=socket.gethostbyname_ex(socket.gethostname())[2][-1]
    ip_port = 80
    qr = qrcode.QRCode(
        version=1,
        error_correction=qrcode.constants.ERROR_CORRECT_L,
        box_size=10,
        border=1,
    )
    qr.add_data('http://'+ip_address+':'+str(ip_port))
    qr.make(fit=True)
    img = qr.make_image(fill_color="black", back_color="white")
    buffered = BytesIO()
    img.save(buffered, format="PNG")
    img_str = base64.b64encode(buffered.getvalue()).decode()
    img_tag = f'<img src="data:image/png;base64,{img_str}" alt="qr-code"/>'
    webbrowser.open('http://'+ip_address+':'+str(ip_port))
    app.run(host='0.0.0.0', port=80)